...
- 后端 `smcore/selection.scan_boll_batch([])` 空候选时调 `run_boll()` 返回归一化列(代码/名称/最新价/信号)；前端"运行完整布林选股(多因子)"按钮 → `POST /api/selection/boll-scan` 空 body 触发。
- `run_boll()` 写出 `Stock-Selection-Boll-{today}.csv` 与 `Stock-Selection-Shared-Seed-{today}.csv`，默认参数读 `smcore.config.defaults`。
