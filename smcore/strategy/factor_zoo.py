...
def format_report(rows: list[dict], meta: dict) -> str:
    ...
    for r in sorted(rows, key=lambda x: (0 if x["verdict"].startswith("存活") else
        ...
        why = r.get("reason", "") or ""
        # 冗余判定先于显著性（见 §四 裁决优先级），故补注自身 p：
        # 否则「与 X 相关 0.99」会被误读成「该因子有预测力」。
        if r["verdict"] == "冗余" and r.get("p_value") is not None:
            why = f"{why}（自身 p={r['p_value']:.2g}，未另判显著档）"
        L.append(f"| {_nm(r)} | {r['family']} | {'多高' if r['prior'] > 0 else '多低'} | "
                 f"{_fmt(r.get('is_mean_ic'))} | {_fmt(r.get('full_mean_ic'))} | "
                 f"{_fmt(r.get('icir'), 3)} | {_fmt(r.get('t_stat'), 2)} | "
                 f"{r.get('p_value', 1):.2g} | {r.get('n_eval')} | {r['verdict']} | {why} |")
    L += [""]
    ...
    L += ["## 四、判读规则（预注册）", "",
          f"- **裁决优先级**（自上而下短路，命中即定档）：冗余 → 样本不足 → 不显著 → IC过弱 → "
          f"分年不稳 → 两段矛盾 → 方向反转 → 存活。注意 **冗余先于显著性判定**：与某个保留因子"
          f"IC 近似复制（|ρ|>{REDUNDANCY_RHO}）的候选，即便自身不显著也会记为「冗余」；此类候选的"
          f"自身 p 已补注在 §二 原因列，不可仅凭「冗余」二字认定其有预测力。",
          f"- **存活** = **Bonferroni(α={BONFERRONI_ALPHA})** 通过 + |均值IC| ≥ {MIN_IC_ABS} + "
          f"分年同号率 ≥ {STABLE_FRAC:.2f} + 发现/验证两段同号 + 与先验同向 + 不与更强因子冗余。",
          f"- **存活(FDR)** = 仅通过 BH-FDR(q={FDR_Q})、未过 Bonferroni —— 更宽松档（p 落在 "
          f"[α/m, q 档] 区间）。抬头「存活 N 个」= 「存活」+「存活(FDR)」两档合计。",
          ...
          "本轮按约定只做**因子有效性筛选**、不改生产选股/配权。以下两项是报告自身**刻意留给下一轮**"
          "的决策，本轮把动作与判据写明，避免「悬而未决」变成「无人认领」：", "",
          "### 1. 「方向反转」16 个（含整个动量家族）如何处置",
          "- 事实：先验「做多高动量」被验证集证伪 —— mom20 −0.0717 / mom40 −0.0722 / mom60 −0.0710，"
          "方向与生产 `momentum` 策略近 3 月 T+10 超额 −3.5% **同向**。",
          "- 建议动作：① 与生产 momentum 策略**取交集核对**（同 universe / 同窗口下 IC 是否同号，"
          "先排除口径差）；② 若确认，则本 universe 上动量应**反用（做空前期强势）或直接置零**；"
          "③ 反用属选股逻辑变更，须走**独立 OOS 门控**（复用 `walk_forward_factor_timing` 的双轨门控），"
          "不得直接改配置。",
          "",
          "### 2. 存活清单如何接入生产",
          "- 现状：21 个存活中**仅 liq20 / illiq20 兼具显著 IC 与正的 TOP50 超额**，且二者属流动性族，"
          "超额被「微盘 + 幸存者 + 不可交易流动性」三重折扣系统性虚高 → **不能直接当选股 alpha**。",
          ...
          "  ③ **并入 factor_timing 因子池**：把 liq20 / illiq20 作为新因子纳入每日「信念 IC」打分，"
          "由既有 overlay 决定其生效与权重（改动最小、与现有生产接线一致）。"]
    return "\n".join(L) + "\n"
