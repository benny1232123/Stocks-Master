...
def apply_sector_weight_cap(
    ...
    与 apply_sector_cap（数量上限）互补：即使单板块只入选 5 只，若这 5 只权重都很高，
    某板块仍可能占组合 40%+。本函数保证「任一板块总仓位 ≤ 全组合总仓位 × max_weight_pct」。

    规则：从高评分往低扫描（df 须已按综合评分降序），累加各板块已选总仓位；
    某板块累计权重将超过上限时跳过该板块后续候选，直至凑满 top_n 或扫完。
    未映射行业（"未知"）不计入任何板块上限，避免全部塌缩进同一桶被误砍。
    不足 top_n 时放宽补满（兜底，保证清单长度）。

    Args:
        df: 已含 '股票代码' 与 '建议仓位%' 列的 DataFrame（按评分降序）
        sector_map: 代码→行业映射
        max_weight_pct: 单板块总仓位占组合比例上限（%）
        top_n: 目标入选总数
    ...
    total_w = float(df["建议仓位%"].sum())
    if total_w <= 0:
        return df.head(top_n), False
    cap = total_w * max_weight_pct / 100.0

    counts: dict[str, float] = defaultdict(float)
    keep_idx: list[int] = []
    for idx, r in df.iterrows():
        ind = industry_of(r["股票代码"], sector_map)
        w = float(r.get("建议仓位%") or 0.0)
        if ind == "未知" or counts[ind] + w <= cap + 1e-9:
            keep_idx.append(idx)
            if ind != "未知":
                counts[ind] += w
        if len(keep_idx) >= top_n:
    ...
    out = df.loc[keep_idx].reset_index(drop=True)
    hit_cap = any(c >= cap - 1e-9 for c in counts.values())
    # 注意：不在此处用「放宽上限补满」兜底——那会让已超限板块重新突破上限，
    # 使权重上限形同虚设。长度不足 top_n 时如实返回（宁可清单短，也不违背中性化约束）。
    return out, hit_cap
