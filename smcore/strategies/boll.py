...
def _compute_report_dates(today: str | None = None):
    """根据信号日月份确定最近的财报日期（与原始脚本一致）。

    优先用 ``today``(YYYYMMDD) 推导月份/年份；缺省时回退到真实当前日期。
    这样在「补跑历史某天」(backfill) 时，财报期与信号日一致，而非取运行日的财报期。
    """
...
def run_boll(
    ...
    *,
    ...
    # ── 4) 布林带技术面 ──
    # 用信号日作为 K 线终点（而非运行日），保证 backfill 时行情拉取到信号日截止。
    if today and len(today) == 8 and today.isdigit():
        end_date = date(int(today[:4]), int(today[4:6]), int(today[6:8]))
    else:
        end_date = date.today()
