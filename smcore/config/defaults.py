...
POSITION_SIZING_CONFIG = {
    ...
    # 误判成普遍超配（每只都 > target 8%）。给出账户总资产/现金后 current_weight
    # 才是真实占比。优先级：account_total > account_cash > account_cash_pct。
    # 对应环境变量（一旦显式给出即覆盖配置项，给 0 等于显式禁用该项）：
    #   ACCOUNT_TOTAL / ACCOUNT_CASH
    # ⚠️ 2026-09-21 用户设定：**持仓市值 + 现金 合计 3 万元** → 用 account_total 固定分母。
    "account_total": 30000.0,           # 账户总资产（= 持仓市值 + 现金），**直接作分母**；
                                        # 现金显示为 总资产 − 持仓市值（随行情自动反推）
    "account_cash": 0.0,                # 备选：只给现金额 → 分母 = 持仓市值 + 现金
    "account_cash_pct": None,           # 备选：只给现金占比（现金/总资产，0~1）
}
